	...
	public void simulate() {
		...
		while (clock < steps){
			...
			for (int i = 0; i < lot.getOccupancy(); i++){
				...
				Spot x = lot.getSpotAt(i);
				int duration = clock - x.getTimestamp();
				if (x != null){
					if (duration >= MAX_PARKING_DURATION){
						leaving = true;
					}
					else{
						leaving = RandomGenerator.eventOccurred(departurePDF.pdf(clock - x.getTimestamp()));

					}
					if (leaving == true){
						x = lot.remove(i);
						outgoingQueue.enqueue(x);
						x = null;
						leaving = false;
					}
				}
			...
			if (incomingQueue.isEmpty() == false){	
				boolean isParked = lot.attemptParking(incomingQueue.peek().getCar(), clock);
